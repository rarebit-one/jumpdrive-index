...
// KnowledgePromotion and the SQLite adapter byte-for-behavior: a proposal HOLDS
// the exact serialized write intent without projecting anything; approving it
// replays that intent through the normal resolve-before-create path, so a proposed
// write and a direct write are identical once approved. Only entity.asserted and
// edge.asserted are proposable.
...
// CLAIMS the proposal (a conditional update on status='pending', so two concurrent
// deciders cannot both apply it) and then replays the held intent through
// AppendEntityFact / AppendEdgeFact. Rejection discards it, writing nothing to the
// graph.
func (s *Store) DecideProposal(ctx context.Context, id store.ProposalID, approve bool, approver domain.PrincipalID) (store.ResolveResult, error) {
	...
	err := s.pool.QueryRow(ctx,
		`SELECT kind, payload, status FROM proposals WHERE id = $1`, string(id)).Scan(&kind, &payload, &status)
	...
	if !approve {
		_, err := s.pool.Exec(ctx,
			`UPDATE proposals SET status='discarded', decided_at=$1, decided_by=$2 WHERE id=$3 AND status='pending'`,
	...
	claim, err := s.pool.Exec(ctx,
		`UPDATE proposals SET status='promoted', decided_at=$1, decided_by=$2 WHERE id=$3 AND status='pending'`,
		ts, string(approver), string(id))
	if err != nil {
		return store.ResolveResult{}, err
	}
	...
	if _, err := s.pool.Exec(ctx,
		`UPDATE proposals SET result_subject=$1 WHERE id=$2`, subject, string(id)); err != nil {
		return store.ResolveResult{}, err
	}
	return result, nil
}

// ListProposals returns pending proposals, optionally scoped to one space, oldest
// first.
func (s *Store) ListProposals(ctx context.Context, f store.ProposalFilter) ([]store.Proposal, error) {
	...
	if f.Space != "" {
		q += ` AND space = ` + ab.add(string(f.Space))
	}
	q += ` ORDER BY created_at, id`

	rows, err := s.pool.Query(ctx, q, ab.vals...) //nolint:gosec // G202: parameterized; only a constant clause + $n placeholder is concatenated
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		var id, kind, proposer, space string
		var payload []byte
